    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
      <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
        ...
        <div v-else-if="tokenStatus === 'valid'">
          ...
          <div class="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div class="text-sm text-gray-600 space-y-1">
              <p><span class="font-medium">Company:</span> {{ companyInfo.name }}</p>
              <p><span class="font-medium">Email:</span> {{ companyInfo.email }}</p>
              <p><span class="font-medium">Location:</span> {{ companyInfo.location }}</p>
              <p><span class="font-medium">Hours:</span> {{ companyInfo.openingTime }} - {{ companyInfo.closingTime }}</p>
            </div>
          </div>
        ...
        <div v-else-if="tokenStatus === 'registered'" class="text-center">
          ...
          <p class="text-green-700 mb-4">Your company account has been created successfully.</p>
          <router-link
            to="/login"
...
export default {
  ...
  methods: {
    async validateToken() {
      ...
      try {
        // Call backend to validate token and get company info
        ...
        try {
          ...
          this.companyInfo = {
            ...
            email: payload.sub || 'company@example.com',
            location: payload.location || 'Location',
            openingTime: payload.opening_time || '09:00',
            closingTime: payload.closing_time || '17:00'
          };
        } catch (decodeErr) {
    ...
    async completeRegistration() {
      ...
      try {
        const payload = {
          ...
          email: this.companyInfo.email, // This should come from the token
          userType: "COMPANY"
        };
